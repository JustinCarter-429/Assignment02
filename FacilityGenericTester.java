...
public class FacilityGenericTester {
	...
	public void testUIDGetPhysicianList() {
		ArrayList<Integer> actualPatients = uIDFacility.getPhysicianList();
		assertEquals(8, actualPatients.size());
	}

	@Test
	public void testUIDGetRecentPatients() {
		ArrayList<CurrentPatientGeneric<Integer>> patients = uIDFacility.getRecentPatients(dates[2]);
		assertEquals(17, patients.size());
	}

	@Test
	public void testUIDUpdatePhysician() {
		uIDFacility.lookupByUHID(uHIDs[1]).updatePhysician(3);
		CurrentPatientGeneric<Integer> patient = uIDFacility.lookupByUHID(uHIDs[1]);
		assertEquals(3, patient.getPhysician());
	}

	public void testUIDSetPhysician() {
		uIDFacility.addPatient(
				new CurrentPatientGeneric<Integer>("Bond", "James", p3id1, 0070070, new GregorianCalendar(2019, 1, 5)));
		uIDFacility.setPhysician(p3id1, 007);
		int newPhysician = uIDFacility.lookupByUHID(p3id1).getPhysician();
		assertEquals(007, newPhysician);
	}
	...
	public void testUHIDGetPhysicianList() {
		ArrayList<UHealthID> actualPatients = uHIDFacility.getPhysicianList();
		assertEquals(8, actualPatients.size());
	}
	...
	public void testUHIDSetPhysician() {
		uHIDFacility.addPatient(new CurrentPatientGeneric<UHealthID>("Peter", "Parker", uHIDs[1], uHIDs[2],
				new GregorianCalendar(2019, 1, 5)));
		uHIDFacility.setPhysician(uHIDs[1], uHIDs[3]);
		int newPhysician = uIDFacility.lookupByUHID(uHIDs[1]).getPhysician();
		assertEquals(uHIDs[3], newPhysician);
	}

	// Add more tests
	@Test // This test make sure the correct exception method is displayed when you try to
			// add a null patient
	public void TestAddPatientUsingNUll() {
		IllegalArgumentException test = new IllegalArgumentException("Please enter a patient to add");
		assertThrows(IllegalArgumentException.class, () -> uHIDFacility.addPatient(null));
		assertEquals("Please enter a patient to add", test.getMessage());
	}

	@Test // FINISH THIS ONE
	public void LookupByPhysicianNameGenericTest() {
		FacilityGeneric<String> facility = new FacilityGeneric<>();
		CurrentPatientGeneric<String> john = new CurrentPatientGeneric<>("John", "Smith", p3id1, "Jeff Slime", p3date1);
		CurrentPatientGeneric<String> ronda = new CurrentPatientGeneric<>("Ronda", "Rousey", p3id2, "Dana White",
				p3date1);
		CurrentPatientGeneric<String> daniel = new CurrentPatientGeneric<>("Daniel", "Cormier", p3id3, "Nick Jonas",
				p3date1);
		facility.addPatient(daniel);
		facility.addPatient(ronda);
		facility.addPatient(john);
		ArrayList<CurrentPatientGeneric<String>> expectedJohn = new ArrayList<>();
		expectedJohn.add(john);
		ArrayList<CurrentPatientGeneric<String>> expectedRonda = new ArrayList<>();
		expectedRonda.add(ronda);
		ArrayList<CurrentPatientGeneric<String>> expectedDaniel = new ArrayList<>();
		expectedDaniel.add(daniel);

		assertEquals(expectedJohn, facility.lookupByPhysician("Jeff Slime"));
		assertEquals(expectedRonda, facility.lookupByPhysician("Dana White"));
		assertEquals(expectedDaniel, facility.lookupByPhysician("Nick Jonas"));
	}

	@Test // I made this Test to ensure that our SetLastVisit method works.
	public void SetLastVisitGenericTest() {
		FacilityGeneric<String> facility = new FacilityGeneric<>();
		CurrentPatientGeneric<String> john = new CurrentPatientGeneric<>("John", "Smith", p3id1, "Jeff Slime", p3date1);
		CurrentPatientGeneric<String> ronda = new CurrentPatientGeneric<>("Ronda", "Rousey", p3id2, "Dana White",
				p3date3);
		...
		facility.addPatient(ronda);
		facility.addPatient(john);
		facility.setLastVisit(p3id1, p3date2);
		facility.setLastVisit(p3id2, p3date1);
		facility.setLastVisit(p3id3, p3date3);
		assertEquals(p3date1, ronda.getLastVisit());
		assertEquals(p3date2, john.getLastVisit());
		assertEquals(p3date3, daniel.getLastVisit());
	}

	@Test // Made this test to make sure The comparator works and orders the names
			// correctly
	public void getOrderedPatientsGenericTest() {
		FacilityGeneric<String> facility = new FacilityGeneric<>();
		CurrentPatientGeneric<String> john = new CurrentPatientGeneric<>("John", "Smith", p3id1, "Jeff Slime", p3date1);
		CurrentPatientGeneric<String> ronda = new CurrentPatientGeneric<>("Ronda", "Rousey", p3id2, "Dana White",
				p3date3);
		...
		facility.addPatient(ronda);
		facility.addPatient(john);
		Comparator<CurrentPatientGeneric<String>> byLastName = (a, b) -> a.getLastName().compareTo(b.getLastName());
		ArrayList<CurrentPatientGeneric<String>> orderedByLas = facility.getOrderedPatients(byLastName);
		assertEquals(daniel, orderedByLas.get(0));
		assertEquals(ronda, orderedByLas.get(1));
		assertEquals(john, orderedByLas.get(2));

	}

	@Test // This Test Sort patients by physician name
	public void getOrderedPatientsSortByPhysicianTest() {
		FacilityGeneric<String> facility = new FacilityGeneric<>();
		CurrentPatientGeneric<String> patient1 = new CurrentPatientGeneric<>("John", "Smith", p3id1, "ADoctor",
				p3date1);
		CurrentPatientGeneric<String> patient2 = new CurrentPatientGeneric<>("Ronda", "Rousey", p3id2, "BDoctor",
				p3date1);
		CurrentPatientGeneric<String> patient3 = new CurrentPatientGeneric<>("Daniel", "Cormier", p3id3, "CDoctor",
				p3date1);
		facility.addPatient(patient1);
		facility.addPatient(patient2);
		facility.addPatient(patient3);
		Comparator<CurrentPatientGeneric<String>> cmp = (a, b) -> a.getPhysician().compareTo(b.getPhysician());
		ArrayList<CurrentPatientGeneric<String>> ordered = facility.getOrderedPatients(cmp);
		assertEquals(patient1, ordered.get(0));
		assertEquals(patient2, ordered.get(1));
		assertEquals(patient3, ordered.get(2));
	}
	...
	public void testOrderedByName() {
		...
		assertEquals(new CurrentPatientGeneric<Integer>("A", "B", p3id1, 7, p3date1), actual.get(1));
		assertEquals(new CurrentPatientGeneric<Integer>("A", "B", p3id2, 7, p3date2), actual.get(0));
		assertEquals(new CurrentPatientGeneric<Integer>("A", "C", p3id3, 7, p3date3), actual.get(2));
		assertEquals(new CurrentPatientGeneric<Integer>("R", "T", p3id4, 7, p3date4), actual.get(3));
	}
